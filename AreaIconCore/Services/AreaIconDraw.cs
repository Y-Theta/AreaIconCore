...
namespace AreaIconCore.Services {
    ...
    internal class AreaIconDraw : SingletonBase<AreaIconDraw> {
        ...
        public Font LoadFont(string familyname, float size = 8, string assemblyname = null, bool system = false) {
            if (system)
                ...
                if (!string.IsNullOrEmpty(assemblyname)) {
                    try {
                        var asb = AppDomain.CurrentDomain.GetAssemblies().First(a => {
                    ...
                    //在资源文件中搜索
                    try { myText = (byte[])Properties.Resources.ResourceManager.GetObject(familyname); }
                    catch {
                        //TODO:字体不存在
        ...
        public Icon StringIcon(int size, string str, Color color, float fontsize, string font = null) {
            Image bufferedimage;
            Font strfont;
            if (_lastsize != fontsize) {
                _lastfont?.Dispose();
                strfont = LoadFont(font, fontsize);
                _lastsize = fontsize;
                _lastfont = strfont;
            }
            else {
